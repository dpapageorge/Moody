...
function EntryModal({ onSave, onClose }) {
  ...
  const handleSave = async () => {
    if (!note.trim() || !polarity) return
    setSaving(true)
    await onSave({ note: note.trim(), polarity, intensity })
    setSaving(false)
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
        {[{ key: 'time', label: 'Time' }, { key: 'good-first', label: 'Good → Bad' }, { key: 'bad-first', label: 'Bad → Good' }].map((o) => (
          <button key={o.key} style={{ ...s.sortBtn, ...(sort === o.key ? s.sortBtnActive : {}) }} onClick={() => setSort(o.key)}>{o.label}</button>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 8 }}>
        {sorted.map((entry) => {
          ...
          return (
            <div
              key={entry.id}
              style={{ aspectRatio: '1', borderRadius: 10, background: isHovered ? 'rgba(0,0,0,0.75)' : bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2, cursor: 'default', transition: 'transform 0.1s', transform: isHovered ? 'scale(1.06)' : 'scale(1)', position: 'relative', overflow: 'hidden' }}
              onMouseEnter={() => setHovered(entry.id)}
              onMouseLeave={() => setHovered(null)}
            >
              {isHovered ? (
                <span style={{ fontSize: 10, color: '#fff', textAlign: 'center', padding: 4, lineHeight: 1.3 }}>
                  {entry.note.length > 30 ? entry.note.slice(0, 28) + '…' : entry.note}
                </span>
              ) : (
                <>
                  <span style={{ fontSize: 11, color: textColor, opacity: 0.75 }}>{entry.polarity === 'good' ? '↑' : '↓'}</span>
                  <span style={{ fontSize: 20, fontWeight: 500, color: textColor, lineHeight: 1 }}>{entry.intensity}</span>
                </>
              )}
            </div>
          )
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 16, justifyContent: 'center' }}>
        {[{ label: 'Good', from: intensityColor('good', 1), to: intensityColor('good', 10) }, { label: 'Bad', from: intensityColor('bad', 1), to: intensityColor('bad', 10) }].map((l) => (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={s.vizCard}>
        ...
        {entries.map((e) => (
          <div key={e.id} style={s.entryItem}>
...
export default function EntryForm({ user }) {
  ...
  const handleSave = async ({ note, polarity, intensity }) => {
    const { data, error } = await supabase
      .from('entries')
      .insert({ note, polarity, intensity, user_id: user.id })
      .select()
      .single()
    if (!error) {
  ...
  return (
    <div style={s.frame}>
      <div style={s.header}>
        <div>
          <h1 style={s.appTitle}>Moody</h1>
          <p style={s.appSub}>{loading ? 'Loading…' : `${entries.length} entries logged`}</p>
        </div>
      ...
      <div style={s.navRow}>
        {[{ key: 'visualized', label: 'Visualized' }, { key: 'analytics', label: 'Analytics' }].map((tab) => (
          <button key={tab.key} style={{ ...s.navTab, ...(view === tab.key ? s.navTabActive : {}) }} onClick={() => setView(tab.key)}>{tab.label}</button>
        ))}
      </div>

      {loading ? (
        <p style={{ textAlign: 'center', color: '#aaa', padding: '2rem 0' }}>Loading entries…</p>
      ) : view === 'visualized' ? (
...
const s = {
  ...
  logoutBtn: { background: '#f1f1f1', color: '#555', border: 'none', borderRadius: 8, padding: '8px 12px', fontSize: 16, cursor: 'pointer' },
  ...
  navTabActive: { background: '#fff', color: '#111', boxShadow: '0 1px 4px rgba(0,0,0,.1)' },
